    ...
    fetch("https://api.rootnet.in/covid19-in/stats/history")
        ...
        .then((res) => {
            ...
            singleRec.regional.forEach(rec => {
                ...
                card.innerHTML = `
                    <h3>${rec.loc}</h3>
                    <p><strong>Confirmed Cases:</strong><span>${rec.totalConfirmed}</span></p>
                    <p><strong>Deaths:</strong> <span>${rec.deaths}</span></p>
                    <p><strong>Recovered:</strong> <span>${rec.discharged}</span></p>
                `;
